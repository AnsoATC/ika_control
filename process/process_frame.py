...
def get_cone_vector(det_image, depth_image, det_model, depth_scale):
    """Detect cones and calculate reference vector based on the two closest cones."""
            ...
            if 0 <= y_c < depth_image.shape[0] and 0 <= x_c < depth_image.shape[1]:
                ...
                if 0.01 <= depth_value <= 10.0:
                    ...
                    if class_name == "traffic_cone":
                        cones.append((x_c, y_c, depth_value))

    # Process the two closest cones
    if len(cones) >= 2:
        cones.sort(key=lambda x: x[2])  # Sort by depth (closest first)
        left_cone = cones[0]
        right_cone = cones[1]
        ...
        cv2.arrowedLine(det_image, (start_x, start_y), (end_x, end_y), (255, 0, 0), 2)  # Blue reference vector
        cv2.circle(det_image, (mid_x, mid_y), 6, (0, 255, 255), -1)  # Yellow reference point
        return np.array([dx, dy]), (mid_x, mid_y), det_image, inference_time
    return None, None, det_image, inference_time

def get_barrier_vector(det_image, depth_image, det_model, depth_scale):
    """Detect barriers and calculate reference vector based on the two closest barriers."""
    h, w = det_image.shape[:2]
    det_results = det_model.predict(det_image, conf=0.5, iou=0.5)
    barriers = []
    ...
    if len(barriers) >= 2:
        barriers.sort(key=lambda x: x[2])  # Sort by depth (closest first)
        ...
        cv2.arrowedLine(det_image, (start_x, start_y), (end_x, end_y), (255, 0, 0), 2)  # Blue reference vector
        cv2.circle(det_image, (mid_x, mid_y), 6, (0, 255, 255), -1)  # Yellow reference point
        return np.array([dx, dy]), (mid_x, mid_y), det_image
    return None, None, det_image

def get_road_mask(seg_image, seg_model):
    """Calculate road mask for illustration."""
...
def process_frame(rgb_image, depth_image, depth_scale, seg_model_path, det_model_path):
    """Process frames for navigation based on cones and barriers."""
    # Load models
    ...
    # Get road mask for illustration
    combined_frame = get_road_mask(combined_frame, seg_model)

    # Determine reference vector (cones first, then barriers)
    cone_vector, cone_ref_point, det_image, inference_time = get_cone_vector(combined_frame, depth_image, det_model, depth_scale)
    if cone_vector is not None:
        reference_vector = cone_vector
        reference_point = cone_ref_point
        combined_frame = det_image  # Use det_image with cone annotations
    else:
        barrier_vector, barrier_ref_point, det_image = get_barrier_vector(combined_frame, depth_image, det_model, depth_scale)
        if barrier_vector is not None:
            reference_vector = barrier_vector
            reference_point = barrier_ref_point
            combined_frame = det_image  # Use det_image with barrier annotations
        else:
